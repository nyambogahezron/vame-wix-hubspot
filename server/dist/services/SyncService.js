...
export class SyncService {
    static async syncContact(integrationId, source, contactData) {
        ...
        const [ledgerEntry] = await db
            .select()
            .from(syncLedger)
            .where(and(eq(syncLedger.integrationId, integrationId), source === "wix"
    ...
    static async syncToHubSpot(integration, userMappings, wixContact, ledgerEntry) {
        const hsProperties = {};
        for (const m of userMappings) {
            if (m.direction === "hs_to_wix")
                continue;
    ...
    static async syncToWix(integration, userMappings, hsContact, ledgerEntry) {
        const wixData = {};
        for (const m of userMappings) {
            if (m.direction === "wix_to_hs")
                continue;
    ...
    static async updateLedger(integrationId, wixContactId, hubspotContactId, source) {
        await db
            .insert(syncLedger)
            .values({
            integrationId,
